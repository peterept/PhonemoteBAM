...
void PhonemoteBAM::createMenu(BAM::BAM_OnButtonPress_func *calibrateCallback) {
    ...
    BAM::menu::info("#c444##-Note: Also set the physical display size" DEFIW);
    BAM::menu::info("#c444##- in the \"Config -> Reality\" menu" DEFIW);
    BAM::menu::info("");
...
DWORD WINAPI PhonemoteBAM::ThreadStart() {
        ...
        // Handle Tracking: Face position
            ...
            if (state.face.valid) {
                ...
                if (cfg.faceTrackingOption == (int)FaceTrackingType::LeftEye) {
                    ...
                    eyeOffset[3] =
                        glm::vec4(state.face.rightEyeX(), state.face.rightEyeY(), state.face.rightEyeZ(), 1.0f);
                }
